...
public class DCommAPI {
  ...
  private static final int DCOMM_OK = 0;
...
////////////////
  ...
  private native int DComm_send_GET(int app_dest, int dev_addr_w,
			int prop_desc_w, int dev_addr, int prop_desc);
  private native int DComm_send_SET(int app_dest, int dev_addr_o,
	int prop_desc_o, int dev_addr, int prop_desc, byte[] value_a);

  private native int DComm_send_NOTIFY(int app_dest,
			int dev_addr, int prop_desc, byte[] value_a);
/*
  private native int DComm_send_EXEC(int app_dest, byte[] action_p);

  private native int DComm_send_MSG(int app_dest, int msg_ctr,
					int data_len, byte[] data_p);
*/
...
//    printError(status);
  ...
  public int sendGet(int app_dest, int dev_addr_w, 
			int prop_desc_w, int dev_addr, int prop_desc)
  {

    return DComm_send_GET(app_dest, dev_addr_w, prop_desc_w,
						dev_addr, prop_desc);
  }


  public int sendSet(int app_dest, int dev_addr_o, int prop_desc_o,
			int dev_addr, int prop_desc, byte[] value_a)
  {
    return DComm_send_SET(app_dest, dev_addr_o, prop_desc_o,
					dev_addr, prop_desc, value_a);
  }


//  public int sendNotify(int app_dest, int dev_addr, int prop_desc, byte[] value_a)
...
/*
  public int sendExec(int app_dest, byte[] action_p)
  {
    return DComm_send_EXEC(app_dest, action_p);
  }


  public int sendMsg(int app_dest, int msg_ctr, int data_len, 
							byte[] data_p)
  {
    return DComm_send_MSG(app_dest, msg_ctr, data_len, data_p);
  }
*/
